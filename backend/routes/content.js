...
router.post('/generate', async (req, res) => {
  try {
    const { prompt, contentType, tone, length, modelConfig } = req.body;
    ...
    // Prepare tailored system prompt for content writing
    const systemPrompt = `You are a professional content writer AI. Generate high-quality content based on the prompt.
    Content Type: ${contentType || 'article'}.
    Tone: ${tone || 'professional'}.
    Length: ${length || 'medium'}.
    Use markdown formatting where appropriate. End with a note that it's AI-generated.`;
